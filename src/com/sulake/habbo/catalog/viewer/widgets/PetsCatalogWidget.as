package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_109,this.onTextWindowEvent);
         ...
         this.var_819 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_313 = [];
         this.var_553 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_819,this);
               if(_loc9_)
               {
                  this.var_553 = _loc9_.sellableBreeds;
                  this.var_313 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_553.length)
         {
            _loc10_ = this.var_553[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_819,_loc10_.id),this.getRaceLocalizationKey(this.var_819,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_819,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1102,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_313)
         {
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_57)
         {
            ...
            if(_loc3_ >= this.var_553.length)
            {
               return;
            }
            this.var_1553 = _loc3_;
            this.updateImage();
         }
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_443 = param1.index;
         if(this.var_443 < 0 || this.var_443 > this.var_313.length)
         {
            this.var_443 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_443 >= 0 && this.var_443 < this.var_313.length)
         {
            _loc7_ = this.var_313[this.var_443] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_553[this.var_1553] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_819,_loc3_.id,_loc2_,AvatarScaleType.const_48,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_295,AvatarAction.const_817);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_443 >= this.var_313.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_313[this.var_443] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_553[this.var_1553] as IBreed;
