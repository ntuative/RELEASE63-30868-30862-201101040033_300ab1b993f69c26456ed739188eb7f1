package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private var var_921:IWindow;
      
      public function EffectView()
      {
         super();
         this.var_179 = new Timer(1000);
         this.var_179.addEventListener(TimerEvent.TIMER,this.onUpdate);
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this.var_179.stop();
            this.var_179.removeEventListener(TimerEvent.TIMER,this.onUpdate);
            this.var_179 = null;
         }
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_171.isActive)
            {
               _loc2_ = this.var_171.secondsLeft / Number(this.var_171.duration);
               this.var_922.width = _loc2_ * this.var_433;
            }
            else
            {
               this.var_922.width = 0;
               this.var_179.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_920.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_920.text;
            if(this.var_171.isActive)
            {
               _loc2_ = this.var_171.secondsLeft;
               ...
               this.var_920.text = _loc1_;
            }
            else
            {
               this.var_920.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_171.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_171.isActive)
         {
         ...
         else
         {
            ...
            this.var_921 = this._window.findChildByName("effect_hilite");
            this.var_921.visible = false;
         }
         this.var_920 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_922 = this._window.findChildByName("loader_bar");
         if(this.var_922 != null)
         {
            this.var_433 = this.var_922.width;
            this.var_179.start();
            this.onUpdate();
         }
         if(this.var_171.icon)
         {
            this.setElementBitmap("effect_icon",this.var_171.icon);
         }
      }
