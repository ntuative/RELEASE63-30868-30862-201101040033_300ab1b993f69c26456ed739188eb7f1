package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2088 = this._config.getKey("interstitial.enabled","TradingYouAreNotAllowedParser") == "ObjectMouseData";
         this.var_2085 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2086 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1529 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_190 != null && this.var_190.running)
         {
            this.var_190.reset();
            return false;
         }
         ...
         if(this.var_1529 > 0 && this.var_1528 >= this.var_1529)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_984;
         if(_loc1_ > this.var_2085)
         {
            ++this.var_1528;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1244 = _loc2_.imageUrl;
         this.var_2087 = _loc2_.clickUrl;
         this.var_984 = getTimer();
         if(this.var_1244 != "")
         {
            ...
            {
               this.var_190 = new Timer(this.var_2086,1);
               this.var_190.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_190.running)
            {
               this.var_190.reset();
            }
            this.var_190.start();
            _loc4_ = new URLRequest(this.var_1244);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_515,0,0,param1.bitmapData,this.var_2087));
            this.var_984 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_487));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1243 = _loc2_.imageUrl;
         this.var_1530 = _loc2_.clickUrl;
         if(this.var_1243 != "" && this.var_1530 != "")
         {
            _loc3_ = this.var_1243;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1243);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_434,0,0,_loc4_,this.var_1530,this.var_985,this.var_983));
         }
      }
