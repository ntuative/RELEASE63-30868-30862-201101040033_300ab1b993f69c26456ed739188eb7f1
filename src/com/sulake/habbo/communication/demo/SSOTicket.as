package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_359:String = "success";
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1415 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1414.init();
            this.var_1414.addEventListener(HabboWeb.const_230,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1415;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1415);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1415);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         else
         {
            try
            {
               this.var_1845 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_2629 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2630 = Boolean(facebookData[1]);
               }
               if(this.var_1845.length > 0)
               {
                  dispatchEvent(new Event(const_359));
               }
               else
               {
