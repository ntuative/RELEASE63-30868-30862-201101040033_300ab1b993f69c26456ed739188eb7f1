package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_746:IFigurePartSet;
      ...
      private const const_1830:int = 13421772;
      
      private const const_1829:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1824 = Math.max(this.var_1824,_loc6_.colorLayerIndex);
            }
         }
         this.var_408 = this.var_61.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_627 != null && !this.var_1825)
            {
               _loc3_ = this.var_627;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_746)
            {
               switch(this.var_746.clubLevel)
               {
                  case HabboClubLevelEnum.const_49:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_56:
                     _loc2_.style = HabboIconType.const_715;
                     break;
                  case HabboClubLevelEnum.const_52:
                     _loc2_.style = HabboIconType.const_742;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1830;
         }
         else
         {
            this._window.color = this.const_1829;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:AvatarFigureContainer = this.var_408.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_312[_loc6_] + "_" + FigureData.const_130;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_611 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1826)
               {
                  ...
                  this.var_1826 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_611.width,this.var_611.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_611.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_611.y;
               if(this.var_1825 && _loc2_.colorLayerIndex > 0)
               {
