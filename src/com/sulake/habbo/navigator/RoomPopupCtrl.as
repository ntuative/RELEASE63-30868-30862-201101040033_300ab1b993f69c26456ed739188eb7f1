package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_100:GuestRoomData;
      
      private var var_1966:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_1966 = new RoomThumbnailRenderer(param1);
         this.var_1153 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_154 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_154.visible = true;
         Util.hideChildren(this.var_154);
         this.refreshRoomName(this.var_154,this.var_100);
         if(this.var_100.ownerName != "" && this.var_100.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_154,!!this.var_100.event ? this.getEventCtg(this.var_100.categoryId) : this.getRoomCtg(this.var_100.categoryId));
         this.refreshRoomDesc(this.var_154,this.var_100);
         this.refreshExtraCont();
         this.refreshInfo(this.var_154,"trading_allowed",this.var_100.allowTrading);
         this.refreshInfo(this.var_154,"doormode_doorbell",this.var_100.doorMode == 1);
         this.refreshInfo(this.var_154,"doormode_password",this.var_100.doorMode == 2);
         Util.moveChildrenToColumn(this.var_154,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_154.height = Util.getLowestPoint(this.var_154);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_154,this.var_100.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_154.findChildByName("roomowner"));
         if(this.var_100.ownerName != "" && this.var_100.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_154.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1153.refreshTags(_loc1_,this.var_100.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_100.score);
         if(this.var_100.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_100.eventCreationTime);
         }
         this.var_1966.refreshThumbnail(_loc1_,this.var_100.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_243,Tabs.const_212,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
