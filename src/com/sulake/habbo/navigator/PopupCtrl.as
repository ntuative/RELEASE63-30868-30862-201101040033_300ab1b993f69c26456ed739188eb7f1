package com.sulake.habbo.navigator
{
   ...
   public class PopupCtrl
   {
      ...
      public function showPopup(param1:IWindow) : void
      {
         if(this.var_50 == null)
         {
            this.var_50 = IWindowContainer(this._navigator.getXmlWindow(this.var_1319));
            this.var_50.visible = false;
            this.var_50.setParamFlag(HabboWindowParam.const_35,true);
            this.var_50.procedure = this.onPopup;
         }
         Util.hideChildren(this.var_50);
         this.refreshContent(this.var_50);
         this.var_50.height = Util.getLowestPoint(this.var_50) + 5;
         var _loc2_:Point = new Point();
         param1.getGlobalPosition(_loc2_);
         this.var_50.x = _loc2_.x + this.var_2541 + param1.width;
         this.var_50.y = _loc2_.y - this.var_50.height * 0.5 + param1.height * 0.5;
         var _loc3_:Point = new Point();
         this.var_50.getGlobalPosition(_loc3_);
         if(_loc3_.x + this.var_50.width > this.var_50.desktop.width)
         {
            this.var_50.x = -this.var_50.width + _loc2_.x + this._popupIndentLeft;
            this.refreshPopupArrows(this.var_50,false);
         }
         else
         {
            this.refreshPopupArrows(this.var_50,true);
         }
         if(!this.var_50.visible)
         {
            this.var_411.reset();
            this.var_411.start();
         }
      ...
      private function onPopup(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(!Util.containsMouse(this.var_50))
            {
               this.closePopup();
            }
         }
