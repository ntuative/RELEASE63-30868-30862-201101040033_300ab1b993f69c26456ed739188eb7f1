package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_79:BitmapData;
      
      private var var_226:IRoomObjectSprite;
      ...
      private var var_982:Number = 1.0;
      
      private var var_548:BitmapData;
      
      private var var_651:ColorTransform;
      
      private var var_1241:ColorTransform;
      
      private var var_1525:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_687:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_651 = new ColorTransform();
         this.var_651.alphaMultiplier = 1;
         this.var_1241 = new ColorTransform();
         this.var_1525 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_91 = this.var_981[param1];
         this.var_808 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1524 >= 0)
         {
            this.var_226 = this._visualization.getSprite(this.var_1524);
            if(this.var_226 && this.var_226.asset)
            {
               if(this.var_226.width <= 1 || this.var_226.height <= 1)
               {
                  return;
               }
               if(this.var_79 == null)
               {
                  this.var_79 = this.var_226.asset.clone();
                  if(this.var_651.alphaMultiplier != 1)
                  {
                     this.var_548 = new BitmapData(this.var_79.width,this.var_79.height,true,4278190080);
                  }
               }
               this.var_2075 = -this.var_226.offsetX;
               this.var_2076 = -this.var_226.offsetY;
               this.var_226.asset = this.var_79;
            }
            if(this.var_79)
            {
               this.var_79.fillRect(this.var_79.rect,4278190080);
            }
            if(this.var_548)
            {
               this.var_548.fillRect(this.var_548.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_91.y * this.var_982;
         }
         return 0;
      }
      ...
      {
         if(!this.var_91 || !this.var_226)
         {
            return;
         }
         if(this.var_79 && this.var_226.asset != this.var_79)
         {
            this.var_226.asset = this.var_79;
         }
         if(this.var_808)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_808 && this.var_91.hasIgnited)
         {
            this.var_808 = true;
         }
         _loc8_ = this.var_1137 * this.var_982;
         this.var_91.update();
         if(this.var_808)
         {
            if(this.var_91.roomObjectSpriteId >= 0)
            ...
            {
               this.var_79.draw(this.var_548,this.var_1525,this.var_651,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_91.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2075 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_982;
               _loc4_ = this.var_2076 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_982;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1241.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_79.draw(_loc7_,this._translationMatrix,this.var_1241,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
                     _loc10_ = new Point(_loc3_ + _loc6_.offsetX,_loc4_ + _loc6_.offsetY);
                     this.var_79.copyPixels(_loc7_,_loc7_.rect,_loc10_,null,null,true);
                  }
               }
               else
               {
                  _loc5_ = new Rectangle(_loc3_ - 1,_loc4_ - 1,2,2);
                  this.var_79.fillRect(_loc5_,4294967295);
               }
            }
            this.var_79.unlock();
         }
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2074 = parseInt(param1.@size);
         this.var_1524 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1137 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_982 = this.var_2074 / 64;
         this.var_687 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_687 = Math.min(this.var_687,1);
         this.var_651.alphaMultiplier = this.var_687;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "TradingYouAreNotAllowedParser" ? false : true;
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "ObjectMouseData" ? true : false;
               _loc21_ = [];
