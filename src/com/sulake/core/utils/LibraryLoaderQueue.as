package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1409:int = 4;
      ...
      {
         return this.var_387.length + this.var_260.length;
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc1_ in this.var_387)
            {
               _loc1_.dispose();
            }
            this.var_260 = null;
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_260.length < const_1409 && this.var_387.length > 0)
            {
               _loc1_ = this.var_387.shift();
               this.var_260.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_387.indexOf(loader);
            if(index > -1)
            {
               this.var_387.splice(index,1);
            }
            index = this.var_260.indexOf(loader);
            if(index > -1)
            {
               this.var_260.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_387)
            {
               if(param2)
               {
