package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      protected var var_1446:uint = 0;
      
      protected var var_259:Function;
      
      protected var var_1186:int;
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_296,this.clientWindowDestroyed);
            this.var_259 = param2;
            this._working = true;
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1186 *= 0.75;
         this.var_1185 *= 0.75;
         if(Math.abs(this.var_1186) <= 1 && Math.abs(this.var_1185) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_259 != null)
         {
            this.var_259(this.var_1186,this.var_1185);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_296,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
